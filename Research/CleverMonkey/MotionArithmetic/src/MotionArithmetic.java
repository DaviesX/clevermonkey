...
public class MotionArithmetic {
        ...
        // 小车两轮间距的一半。
        protected static final float k_carWidthHalf = 0.07f;

        // 中心点本底速度(m/s)。
        protected static float m_vBase = 0.05f;

        // 接口。
        // 通过提供的运动目标点及运动参数，算出对应的运动学控制参数。
        public static void GetMotionArgu(Vec2 vLeft, Vec2 vRight, final Vec2 target,
                ...
                if (resultType == Tracker.ResultType.Stop) {
                        vLeft.set(0, 0);
                        vRight.set(0, 0);
                } else if (resultType == Tracker.ResultType.Unk) {
                        vLeft.set(0, m_vBase);
                        vRight.set(0, m_vBase);
                }
        }

        public static void GetMotionArgu2(Vec2 vLeft, Vec2 vRight, final Vec2 target,
                                          Tracker.ResultType resultType) {
